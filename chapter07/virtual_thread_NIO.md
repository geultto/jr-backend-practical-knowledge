...
- jvm이 플랫폼 스레드 감시하다가 blocking상태에 들어가면 unmounted합니다.
...
스레드가 왜 한번에 하나씩만 동작될까?
os 스케줄러가 ready queue에서 다음 스레드 선택?
1. CPU 코어 하나의 구조:
```
   ┌─────────────────────────┐
...
await function() -> promise(성공, 실패 여부를 담은 객체) return 받을 떄까지 기다림
event lope를 돌면서 작업 완료될 때까지 기다림

await 을 쓰지 않으면 pending상태 promise 객체 return
